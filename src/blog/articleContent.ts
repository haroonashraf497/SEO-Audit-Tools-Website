/**
 * Article bodies for the built-in blog posts — a separate, lazily-imported
 * chunk.
 *
 * The homepage and every tool page used to carry these four long-form articles
 * (~87 KiB of text) inside the entry bundle even though only a blog article
 * page ever shows them. They now live here and are fetched when a blog post is
 * opened (the router warms this chunk while preparing a /blog/<slug>
 * navigation, so the article still renders complete on its first paint).
 *
 * Generated from the article sources — keep the metadata in src/blog/articles-*
 * and the bodies here in step.
 */
export const ARTICLE_CONTENT: Record<string, string> = {
  "inp-core-web-vital-fix": `
...
`,
  "wordpress-plugins-slowing-site": `
