        ...
        <>
          ...
          {outs.length === 0 && (
            <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
              ...
              {mode === 'range' && <div><label className="text-sm font-semibold text-slate-700 block mb-1">Pages to extract (e.g. 1-3,5,8-10)</label><input aria-label="Pages to extract" value={ranges} onChange={e => setRanges(e.target.value)} className="w-full p-3 rounded-xl border border-slate-300 text-sm font-mono" /><p className="text-xs text-slate-500 mt-1">{f.info ? `${parseRanges(ranges, f.info.pages).length} of ${f.info.pages} pages selected` : ''}</p></div>}
              {mode === 'chunks' && <div><label className="text-sm font-semibold text-slate-700 block mb-1">Pages per file</label><input aria-label="Pages per file" type="number" min={1} value={chunk} onChange={e => setChunk(Math.max(1, Number(e.target.value)))} className="w-32 p-3 rounded-xl border border-slate-300 text-sm" /><p className="text-xs text-slate-500 mt-1">{f.info ? `Creates ${Math.ceil(f.info.pages / chunk)} files` : ''}</p></div>}
          ...
          {!out && (
            <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
              <div className="grid md:grid-cols-2 gap-4">
                <div><label className="text-sm font-semibold text-slate-700 block mb-1">Open password (required)</label><div className="relative"><input aria-label="Open password" type={show ? 'text' : 'password'} value={userPw} onChange={e => setUserPw(e.target.value)} className="w-full p-3 pr-16 rounded-xl border border-slate-300 text-sm" placeholder="Needed to open the file" /><button type="button" onClick={() => setShow(!show)} className="absolute right-2 top-1/2 -translate-y-1/2 text-xs font-semibold text-slate-500">{show ? 'Hide' : 'Show'}</button></div>
        ...
        <>
          ...
          {!out && (
            <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
              ...
              <div><label className="text-sm font-semibold text-slate-700 block mb-1">Open password (only if required)</label><input aria-label="Open password" type="password" value={pw} onChange={e => setPw(e.target.value)} className="w-full p-3 rounded-xl border border-slate-300 text-sm" placeholder="Leave blank if the file opens without a password" /></div>
