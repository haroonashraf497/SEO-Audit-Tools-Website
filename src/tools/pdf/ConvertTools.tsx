...
const TextOptions: React.FC<{ o: { size: string; landscape: boolean; fontName: 'Helvetica' | 'TimesRoman' | 'Courier'; fontSize: number; margin: number; lineHeight: number; pageNumbers: boolean }; set: (k: string, v: unknown) => void }> = ({ o, set }) => (
  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
    <div><label className="text-xs font-semibold text-slate-600 block mb-1">Page size</label><select value={o.size} onChange={e => set('size', e.target.value)} className="w-full p-2.5 rounded-lg border border-slate-300 text-sm bg-white">{Object.keys(PAGE_SIZES).map(s => <option key={s}>{s}</option>)}</select></div>
    <div><label className="text-xs font-semibold text-slate-600 block mb-1">Font</label><select value={o.fontName} onChange={e => set('fontName', e.target.value)} className="w-full p-2.5 rounded-lg border border-slate-300 text-sm bg-white"><option value="Helvetica">Helvetica (sans)</option><option value="TimesRoman">Times (serif)</option><option value="Courier">Courier (mono)</option></select></div>
    <div><label className="text-xs font-semibold text-slate-600 block mb-1">Font size {o.fontSize}pt</label><input type="range" min={8} max={18} value={o.fontSize} onChange={e => set('fontSize', Number(e.target.value))} className="w-full accent-indigo-600" /></div>
    <div><label className="text-xs font-semibold text-slate-600 block mb-1">Margins {Math.round(o.margin / 72 * 25.4)}mm</label><input type="range" min={36} max={90} value={o.margin} onChange={e => set('margin', Number(e.target.value))} className="w-full accent-indigo-600" /></div>
    <div><label className="text-xs font-semibold text-slate-600 block mb-1">Line spacing {o.lineHeight.toFixed(1)}</label><input type="range" min={1.1} max={2} step={0.1} value={o.lineHeight} onChange={e => set('lineHeight', Number(e.target.value))} className="w-full accent-indigo-600" /></div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid lg:grid-cols-[1fr_260px] gap-4">
        <div className="space-y-3"><input value={title} onChange={e => setTitle(e.target.value)} placeholder="Document title (optional)" className="w-full p-3 rounded-xl border border-slate-300 text-sm" /><textarea value={text} onChange={e => { setText(e.target.value); setOut(null); }} rows={14} placeholder="Type or paste your text here… Lines starting with # become headings, lines starting with - become bullets." className="w-full p-4 rounded-xl border border-slate-300 text-sm outline-none focus:border-indigo-500" /></div>
    ...
    <div className="space-y-5">
      ...
      {!text ? <DropZone accept=".docx,.txt,.md,.rtf,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onFiles={onFile} label="Select a Word document (.docx)" hint="Also accepts .txt / .md · parsed locally, nothing uploaded" /> : (
        <>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4"><input value={title} onChange={e => setTitle(e.target.value)} placeholder="Document title" className="w-full p-3 rounded-xl border border-slate-300 text-sm" /><TextOptions o={o} set={(k, v) => setO({ ...o, [k]: v })} /><Btn onClick={run} className="w-full py-4 text-base">Convert to PDF →</Btn></div>
        </>
      )}
          ...
          {imgs.length === 0 && (
            <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
              <div className="grid sm:grid-cols-3 gap-4">
                ...
                {fmt === 'image/jpeg' && <div><label className="text-sm font-semibold text-slate-700 block mb-1">JPG quality {Math.round(quality * 100)}%</label><input type="range" min={0.5} max={1} step={0.05} value={quality} onChange={e => setQuality(Number(e.target.value))} className="w-full accent-indigo-600 mt-2" /></div>}
              </div>
        ...
        <>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
            <div className="grid sm:grid-cols-4 gap-3">
              <div><label className="text-xs font-semibold text-slate-600 block mb-1">Page size</label><select value={size} onChange={e => setSize(e.target.value)} disabled={fit === 'original'} className="w-full p-2.5 rounded-lg border border-slate-300 text-sm bg-white">{Object.keys(PAGE_SIZES).map(s => <option key={s}>{s}</option>)}</select></div>
              <div><label className="text-xs font-semibold text-slate-600 block mb-1">Orientation</label><select value={orient} onChange={e => setOrient(e.target.value as typeof orient)} className="w-full p-2.5 rounded-lg border border-slate-300 text-sm bg-white"><option value="auto">Auto (per image)</option><option value="portrait">Portrait</option><option value="landscape">Landscape</option></select></div>
              <div><label className="text-xs font-semibold text-slate-600 block mb-1">Image fit</label><select value={fit} onChange={e => setFit(e.target.value as typeof fit)} className="w-full p-2.5 rounded-lg border border-slate-300 text-sm bg-white"><option value="fit">Fit inside page</option><option value="fill">Fill page (crop)</option><option value="original">Page = image size</option></select></div>
              <div><label className="text-xs font-semibold text-slate-600 block mb-1">Margin {margin}pt</label><input type="range" min={0} max={72} value={margin} onChange={e => setMargin(Number(e.target.value))} className="w-full accent-indigo-600 mt-2" /></div>
            </div>
...
export const PptToPdf: React.FC = () => {
  ...
  return (
    <div className="space-y-5">
      <PrivacyNote />
      <div className="grid lg:grid-cols-[1fr_260px] gap-4">
        <textarea value={text} onChange={e => { setText(e.target.value); setOut(null); }} rows={14} className="w-full p-4 rounded-xl border border-slate-300 text-sm font-mono outline-none focus:border-indigo-500" />
...
export const ExcelToPdf: React.FC = () => {
  ...
  return (
    <div className="space-y-5">
      <PrivacyNote />
      <div className="grid lg:grid-cols-[1fr_260px] gap-4">
        <div className="space-y-3"><input value={title} onChange={e => setTitle(e.target.value)} placeholder="Table title" className="w-full p-3 rounded-xl border border-slate-300 text-sm" /><textarea value={raw} onChange={e => { setRaw(e.target.value); setOut(null); }} rows={10} placeholder="Paste cells from Excel/Sheets (tab-separated) or CSV…" className="w-full p-4 rounded-xl border border-slate-300 text-sm font-mono outline-none focus:border-indigo-500" /></div>
