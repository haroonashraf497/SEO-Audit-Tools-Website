// Compact, dependency-free MD5 implementation (RFC 1321)
export function md5(input: string): string {
  const rh = (n: number): string => {
    let s = '';
    for (let j = 0; j <= 3; j++) {
  ...
  const rl = (n: number, c: number): number => (n << c) | (n >>> (32 - c));
  const cm = (q: number, a: number, b: number, x: number, s: number, t: number): number =>
    ad(rl(ad(ad(a, q), ad(x, t)), s), b);
  ...
  // Append 0x80 bit, then pad with zeros until length ≡ 14 (mod 16) words
  x[len >> 2] = (x[len >> 2] || 0) | (0x80 << ((len % 4) * 8));
  while (x.length % 16 !== 14) x.push(0);
  // Append 64-bit little-endian bit length (low word, then high word)
  x.push((len * 8) >>> 0);
  x.push(Math.floor(len / 0x20000000) >>> 0);
